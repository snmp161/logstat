...
| `logstat_redis_up` | gauge | — | `1`, если на этом скрейпе Redis удалось прочитать |
...
При `metrics.path: /` корень занимает экспозиция метрик, и статус-страницы нет.
...
На эндпоинте нет ни TLS, ни аутентификации, ни rate-limit — это сознательно вне рамок. Дефолтный `listen` слушает только петлю; выставить его наружу — осознанное действие оператора, желательно за firewall или reverse proxy.
